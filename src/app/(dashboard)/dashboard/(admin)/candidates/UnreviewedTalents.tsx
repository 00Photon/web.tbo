  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        setAnchorEl(Array(pendingApplications.length).fill(null));
      } catch (err) {
  ...
  return (
    ...
    >
      <CardContent sx={{ p: (theme) => theme.spacing(3) }}>
        {!smallScreen && (
          <Typography variant="h6">Talents Under Review</Typography>
        )}
...
export default UnderReviewTalents;